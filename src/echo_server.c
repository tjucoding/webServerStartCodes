...
static void map_uri_to_path(const char *uri, char *path, size_t path_sz) {
    if (strcmp(uri, "/") == 0) {
    ...
    snprintf(path, path_sz, "static_site/%s", u);
}

static size_t handle_get_head(int fd, const HttpRequest *req, int *status_out) {
    char path[4096] = {0};
    map_uri_to_path(req->parsed->http_uri, path, sizeof(path));
